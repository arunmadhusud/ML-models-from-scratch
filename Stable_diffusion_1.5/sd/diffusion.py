...
class UNET_AttentionBlock(nn.Module):
    ...
    def __init__(self, n_head:int, n_embed: int, d_context = 768):
        super().__init__()
        self.channels = n_head * n_embed

        self.groupnorm = nn.GroupNorm(32, self.channels, eps=1e-6)
        self.conv_input = nn.Conv2d(self.channels, self.channels, kernel_size=3, padding=1)

        self.layernorm_1 = nn.LayerNorm(self.channels)
        self.attention_1 = SelfAttention(n_head, self.channels, in_proj_bias=False)
        self.layernorm_2 = nn.LayerNorm(self.channels)
        self.attention_2 = CrossAttention(n_head, self.channels, d_context, in_proj_bias=False)
...
class Diffusion(nn.Module):
    ...
    def forward(self, latent: torch.Tensor, context: torch.Tensor, time: torch.Tensor):
        ...
        # context: (Batch, sequence_length,n_embed = 768)
        # time: (1,320)
